...
public class MoveElementToEnd {
	public static List<Integer> moveElementToEnd(List<Integer> array, int toMove) {
		...
		// move j as long as it points to toMove ; j looks for the other than toMove
		// element always; elements to the right of j are all toMove elements
		// move i as long as it doesnot point to toMove; i looks for the toMove element
		// always to swap it to the end;
		// if i crosses j; then i need not worry about to swap the elements any more
		// because all the elements are toMove elements
		while (i < j) {
	...
	public static List<Integer> moveElementToEndMySol1(List<Integer> array, int toMove) {
		...
		while (left < right) {
			...
			while (left < right && array.get(left) != toMove)
				left++;
