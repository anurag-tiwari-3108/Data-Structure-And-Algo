package com.codex.dsa.arrays;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

// Subset Sum 1 prob with one additional condition to avoid any duplicate
// We are following another approach here to avoid duplicate i.e sort the array and validate if current_val == current-1_val
public class SubsetSum2 {

    public List<List<Integer>> subsetSum(int[] arr) {
        Arrays.sort(arr);
        List<List<Integer>> ans = new ArrayList<>();
        backtrack(0, arr, new ArrayList<>(), ans);
        return ans;
    }

    private void backtrack(int ind, int[] arr, List<Integer> ds, List<List<Integer>> ans) {
        ans.add(new ArrayList<>(ds));
        for (int i = ind; i < arr.length; i++) {
            if (i != ind && arr[i] == arr[i - 1]) continue;
            ds.add(arr[i]);
            backtrack(i + 1, arr, ds, ans);
            ds.remove(ds.size() - 1);
        }
    }

    public static void main(String[] args) {
        SubsetSum2 solver = new SubsetSum2();
        int[] arr = {1, 2, 2};
        List<List<Integer>> result = solver.subsetSum(arr);
        System.out.println("All unique subsets:");
        for (List<Integer> subset : result) {
            System.out.println(subset);
        }
    }
}
